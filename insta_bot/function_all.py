...
async def all_function(login, password, hashtag, group_name, user_id):
    ...
    await close_browser()
    await bot.send_message(user_id, f"Готово! Количество пройденных постов: {count_completed}\nФункция: *Лайк, репост, сохранение*",
                           parse_mode="Markdown")
...
async def close_browser_all():
    await close_browser()

if __name__ == '__main__':
    asyncio.run(all_function("login", "password", "hashtag", 0))  # тестовая функция
